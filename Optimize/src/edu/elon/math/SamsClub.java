...
public class SamsClub extends Function {
	 ...
	 * Generated ID
	 */
	private static final long serialVersionUID = 2288128778496122575L;
	...
	/**
	 * Constructor initializes initial input point to ArrayList<Double> passed
	 * in as a parameter
	 * 
	 * @param inputs
	 *            ArrayList<Double> representing values for initial design
	 *            point.
	 * @throws RemoteException
	 */
	...
	/**
	 * Constructor that initializes starting design point from values passed in
	...
	/**
	 * Determines if two SamsClub instances are the same based on having the
	 * same values and names for each input
	 * 
	...
	public Double evaluate() throws RemoteException {
		...
		double cost = 60.0 / (1 + Math.pow(x + 1, 2) + Math.pow(y - 3, 2))
				+ 20.0 / (1 + Math.pow(x - 1, 2) + Math.pow(y - 3, 2)) + 30.0
